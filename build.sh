...
# Check current version for Linux amd64 (fallback if no caddy found)
test -f caddy_amd64 && current_version=$(./caddy_amd64 version | cut -d" " -f 1 | cut -d"v" -f 2) || current_version='caddy not found'
